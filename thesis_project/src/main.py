...
def process_audio_file(input_file: str, effect: AudioEffect, selected_preset: str, output_dir: Path) -> tuple[
                                                                                          np.ndarray, np.ndarray, int] | None:
    """
    Applica un effetto audio a un file e restituisce i segnali e il samplerate.

    Parametri in input:
    - input_file: nome file
...
def get_user_choice() -> tuple[str, str] | None:
    ...
    Parametri in output:
    - selected_effect, selected_preset : il nome dell'effetto selezionato e quello del preset selezionato
    """
...
def main():
    ...
    result = process_audio_file(str(input_file), effect, selected_preset, output_dir)
    if result is None:
        return
